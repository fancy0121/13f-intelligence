...
METRICS = {
    T("已核验申报主体新增计数", "Verified filing entities NEW"): "independent_new_manager_count",
    T("已核验申报主体增持计数", "Verified filing entities ADD"): "independent_add_manager_count",
    T("已核验申报主体减持计数", "Verified filing entities REDUCE"): "independent_reduce_manager_count",
    T("已核验申报主体退出计数", "Verified filing entities EXIT"): "independent_exit_manager_count",
...
def run() -> None:
    ...
    with st.expander(T("如何使用本页", "How to use this page")):
        st.markdown(
            "1. " + T("输入关键字筛选排序指标，然后点击指标按钮（例如「已核验申报主体增持计数」）。",
                      "Type to filter the ranking metric, then click it (e.g. Verified filing entities ADD).") + "\n"
            "2. " + T("列表按该指标从高到低展示证券，并同时显示计数、解析状态、活动状态。",
                      "Securities are ranked by that metric, showing counts, resolution and activity.") + "\n"
            "3. " + T("这是中性事实排序：数字是「多少个已核验申报主体报告了该动作」，不是建议你买或卖。",
                      "This is a neutral factual ranking: the number of verified filing entities "
    ...
    st.caption(
        B(
            "说明：计数为最新报告季度内“已核验申报主体”的动作数量；重复计数为连续 ≥2 个报告季度同方向"
            "（缺失季度中断）。身份未验证/未解析证券仍会显示其解析状态，不做隐藏过滤。该页仅为中性事实排序，"
            "不构成任何“最佳/机会”含义。",
            "Counts are actions by verified filing entities in the latest quarter; repeated counts "
