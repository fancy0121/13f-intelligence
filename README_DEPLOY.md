...
> **2026-09-07 审计：以下为旧部署说明，当前禁止据此发布新版。**
> 旧 Docker 构建期下载流程不满足原始凭证、真实 Gate 和不可变发布要求。
> 当前改动仅通过本地离线验证，生产发布仍 `NOT_READY`。应按已批准的
> `docs/superpowers/plans/2026-09-05-unattended-refresh-deploy-master.md` 顺序验收。
> 公开版当前设计为服务端会话内存隔离，不再向临时目录保存持仓/研究记录。
> 不要以旧镜像运行成功或健康检查 `ok` 代替数据验收；完整状态见
> [源码候选版状态](reports/releases/source-preview-2026-09-10/README.md)。
...
- 因此采用「**Docker 构建期建库**」：镜像构建时运行
  ...
  这是已停用的旧构建设计，不可用于本源码候选版。当前公开模式的组合与研究记录仅保留在
  各自服务端会话内存，不写服务端文件，也不是浏览器本地存储；不保证跨会话恢复或断网立即清除。
- 数据仍是延迟的13F季度披露，修订等可能更晚，不是行情实时。
...
- 总览第一屏是 **DATA STATUS**：最新报告季度、filing 日期、本地更新时间、机构覆盖、
  陈旧、修订、身份解析状态。
- 「我的组合」在公网上按浏览器会话隔离。每个访客从空列表开始，其他访客看不到；
  新会话或容器重建后会清空。
