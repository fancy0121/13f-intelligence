...
> **DEPRECATED / NOT_READY：以下旧部署流程不能用于当前源码候选版。**
> 旧Dockerfile仍在构建期联网运行旧数据命令；当前CI已禁止镜像发布。
> 不要运行本文的更新、删除容器或重建步骤；正式部署需完成真实Gate和受控发布计划。
> 当前状态见[源码候选说明](../reports/releases/source-preview-2026-09-10/README.md)。
...
当前公开模式不修改证据数据库，也不从页面实时请求SEC；组合与研究记录只使用
服务端会话内存，不写服务端文件，不是浏览器本地存储。季度披露有延迟，修订等可能更晚，
不是行情实时。上图是旧架构记录，不代表本候选版已经部署。
...
公开模式没有登录系统。组合与研究记录按服务端会话内存隔离，不写服务端文件，
也不是浏览器本地存储；不保证跨会话恢复或断网立即清除。需要长期保存时，请下载研究记录；持仓长期保存功能
需要以后增加账户与权限系统，不属于当前版本。
